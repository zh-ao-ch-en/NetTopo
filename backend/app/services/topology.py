...
from app.seed import build_seed_topology_payload
from app.utils import now, fmt
...
def reset_topology(db: Session) -> dict:
    """将拓扑重置为初始种子（复用种子数据，仅影响拓扑图）。"""
    topo_in = TopologyIn(**build_seed_topology_payload())
    return save_topology(db, topo_in)