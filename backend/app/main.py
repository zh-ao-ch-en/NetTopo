...
app = FastAPI(title="网络实验室拓扑与设备管理系统 API", version="0.1.0", lifespan=lifespan, docs_url=None, redoc_url=None, openapi_url=None)
...
async def unhandled_exception_handler(request: Request, exc: Exception):
    # 只记摘要，避免把堆栈与用户输入写入日志；排查时可临时加 exc_info=True
    logger.error(
