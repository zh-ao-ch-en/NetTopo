...
from app.utils import now, offset_date, offset_datetime, today

# ---- 用户：id, username, password, display_name, role, email, phone, created_at ----
# 生产只播种一个 admin 账号，避免公开弱口令的 teacher/student 被利用；
# 其它账号由用户通过「用户管理」页面自行创建，重启后仍会保留。
USERS = [
    ("u-001", "admin", "admin123", "系统管理员", "admin", "admin@lab.edu.cn", "13800000001", datetime(2023, 1, 1)),
]
...
def _build_device(d: dict) -> Device:
    dev = Device(
        ...
        metrics=d["metrics"],
        remark=d["remark"],
        created_at=offset_datetime(d["created_days"]),
        updated_at=offset_datetime(d["updated_days"]),
    )
    for p in d["ports"]:
...
def seed_all(db: Session) -> None:
    ...
    db.flush()  # 设备先入库，供拓扑节点/告警的外键引用

    db.add(Topology(id="topo-main", name="网络实验室主拓扑", updated_at=now()))
    db.flush()  # 拓扑先入库，供拓扑节点/连线的外键引用
    ...
    for aid, dev_id, dev_name, level, msg, time_, resolved in ALERTS:
        db.add(
            Alert(
                ...
                level=level,
                message=msg,
                time=datetime.combine(today(), dtime.fromisoformat(time_)),
                resolved=resolved,
            )
        )
...
def build_seed_topology_payload() -> dict:
    """初始拓扑的契约格式数据，供「重置拓扑」接口复用（仅重置拓扑图，不动设备台账）。"""
    nodes = []
    for nid, label, ntype, x, y in TOPOLOGY_NODES:
        size = DEFAULT_NODE_SIZE[ntype]
        nodes.append(
    ...
    edges = [
        {"id": eid, "source": src, "target": tgt, "label": label, "style": style}
        for eid, src, tgt, label, style in TOPOLOGY_EDGES
    ]
    return {
        "id": "topo-main",
