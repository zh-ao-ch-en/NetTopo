...
from app.constants import DEFAULT_NODE_SIZE, DEVICE_FIELD_MAP
...
def _upsert_device(db: Session, d: dict) -> Device:
    ...
    elif (d.get("assetNo") or "").strip():
        # 无 mac 的设备以资产编号兜底去重，避免重复导入产生僵尸空值设备
        dev = db.query(Device).filter(Device.asset_no == d["assetNo"].strip()).first()
    if dev is None:
    ...
    dev.created_at = dev.created_at or now()
    dev.updated_at = now()
    return dev
