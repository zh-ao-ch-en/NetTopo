...
import type { Device, DeviceStatus, DeviceType, NodeType, TopologyEdge, TopologyNode } from '@/types'
import { DEFAULT_NODE_SIZE, DEVICE_TYPE_NAME } from '@/utils/constants'
import { genId } from '@/utils/id'
import { getTopology, resetTopology as apiResetTopology, saveTopology } from '@/api/topology'
...
function updateNodeSize(axis: 'width' | 'height', v: number) {
  if (selectedNode.value && v) {
    const min = axis === 'width' ? 88 : 44
    selectedNode.value[axis] = Math.max(min, Math.round(v))
    scheduleSave()
  }
}
...
function normalizeImported(raw: unknown): { nodes: TopologyNode[]; edges: TopologyEdge[] } {
  if (!raw || typeof raw !== 'object') throw new Error('文件内容不是有效的 JSON 对象')
  const d = raw as Record<string, unknown>
  if (!Array.isArray(d.nodes) || !Array.isArray(d.edges)) throw new Error('缺少 nodes / edges 数组')
  const seen = new Set<string>()
  const nodes: TopologyNode[] = []
  for (const item of d.nodes) {
    if (!item || typeof item !== 'object') throw new Error('存在非法节点')
    const n = item as Record<string, unknown>
    const id = typeof n.id === 'string' ? n.id : ''
    if (!id) throw new Error('存在缺少 id 的节点')
    if (seen.has(id)) throw new Error(`节点 id 重复：${id}`)
    seen.add(id)
    const num = (k: string) => (typeof n[k] === 'number' ? (n[k] as number) : Number(n[k]) || 0)
    nodes.push({
      id,
      deviceId: typeof n.deviceId === 'string' ? n.deviceId : undefined,
      label: typeof n.label === 'string' ? n.label : id,
      type: (typeof n.type === 'string' ? n.type : 'pc') as NodeType,
      x: num('x'),
      y: num('y'),
      width: num('width'),
      height: num('height'),
      status: typeof n.status === 'string' ? (n.status as DeviceStatus) : undefined,
    })
  }
  const edges: TopologyEdge[] = []
  for (const item of d.edges) {
    if (!item || typeof item !== 'object') throw new Error('存在非法连线')
    const e = item as Record<string, unknown>
    const source = typeof e.source === 'string' ? e.source : ''
    const target = typeof e.target === 'string' ? e.target : ''
    if (!source || !target) throw new Error('连线缺少有效的 source/target')
    edges.push({
...
function onImportFile(e: Event) {
  ...
  reader.onload = () => {
    try {
      ...
      const { nodes: ns, edges: es } = normalizeImported(data)
      cancelPendingSave()
      if (typeof data.name === 'string') topoName.value = data.name
      nodes.value = ns
      edges.value = es
      selectedId.value = null
      selectedEdgeId.value = null
...
async function resetTopology() {
  try {
    await ElMessageBox.confirm('确定重置为初始拓扑吗？当前画布内容将被覆盖。', '重置确认', {
      type: 'warning',
  ...
  try {
    const topo = await apiResetTopology()
    topoName.value = topo.name
    nodes.value = topo.nodes
    edges.value = topo.edges
    selectedId.value = null
