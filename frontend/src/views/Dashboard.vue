<template>
  <div>
    ...
    <p class="page-subtitle">设备运行状态总览与告警信息</p>
      ...
      <el-table :data="alerts" v-loading="loading" style="width: 100%">
        <el-table-column label="级别" width="90">
          <template #default="{ row }">
            <el-tag :type="ALERT_LEVEL_TYPE[row.level as AlertLevel]" size="small" effect="dark">{{ levelName(row.level) }}</el-tag>
          </template>
        </el-table-column>
...
import type { Alert, AlertLevel, StatusSummary } from '@/types'
import { listAlerts, resolveAlert, getStatusSummary } from '@/api/monitor'
import { useAuthStore } from '@/stores/auth'
import { canEdit } from '@/utils/permission'
import { ALERT_LEVEL_NAME, ALERT_LEVEL_TYPE } from '@/utils/constants'
